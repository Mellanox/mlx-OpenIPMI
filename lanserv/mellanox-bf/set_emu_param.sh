...
if [ "$i2cbus" != "NONE" ]; then
	...
	if [ -f $IPMB_RETRY_FLAG ]; then
		...
		retries=$(cat $IPMB_RETRY_FLAG)
		# Calculate the retry interval. Increase by 6 minutes for each retry.
		retry_interval=$((6 * retries))
		# After calculating the retry_interval, check if it exceeds the maximum allowed value of 59
		# This is because the max time in ipmb_update_timer is 59
		if [ $retry_interval -gt 59 ]; then
			retry_interval=59
		fi
