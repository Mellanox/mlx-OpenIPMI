...
I2C1_NEW_DEV=/sys/bus/i2c/devices/i2c-1/new_device

if [ "$bffamily" = "BlueSphere" ] || [ "$support_ipmb" != "0" ]; then
	# Instantiate the ipmb-dev device. The driver is already loaded by default.
	if [ ! -c "/dev/ipmb-1" ]; then
		echo ipmb-dev $IPMB_DEV_INT_ADD > $I2C1_NEW_DEV
	fi

	if ! grep -q "ipmb-1" /etc/ipmi/mlx-bf.lan.conf; then
		echo "  ipmb 1 ipmb_dev_int /dev/ipmb-1" >> /etc/ipmi/mlx-bf.lan.conf
	fi
	...
	if [ ! "$(lsmod | grep ipmb_host)" ]; then
		...
		echo ipmb-host $IPMB_HOST_ADD > $I2C1_NEW_DEV
	fi
fi
...
if [ "$t" = "$fru_timer" ]; then
	...
	add_fru "emmc_info" 4
	add_fru "qsfp0_eeprom" 5
	add_fru "qsfp1_eeprom" 6
	add_fru "dimms_ce_ue" 8
